...
cd /inery.contracts/build/tests
...
EXIT_STATUS=$?
[[ "$EXIT_STATUS" == 0 ]] && set -e
mv /inery.contracts/build/tests/Testing/$(ls /inery.contracts/build/tests/Testing/ | grep '20' | tail -n 1)/Test.xml /artifacts/Test.xml
