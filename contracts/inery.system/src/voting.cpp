...
namespace inerysystem {
   ...
   void system_contract::register_producer( const name& producer, const inery::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      auto prod = _producers.find( producer.value );
      const auto ct = current_time_point();
      accounts_activity accountsact(get_self(), producer.value);
      auto prod_act = accountsact.find( producer.value );
      inery::public_key producer_key{};
      ...
      // for producer activity info in transfer
      if ( prod_act != accountsact.end()){
         accountsact.modify( prod_act, producer, [&]( account_activity& prd_info ){
            prd_info.is_active_prod  = true;
         });
      } else {
         accountsact.emplace( producer, [&](account_activity& prd_info ){
            prd_info.owner = producer;
            prd_info.is_active_prod = true;
         });
      }
   }
   ...
   void system_contract::regproducer2( const name& producer, const inery::block_signing_authority& producer_authority, const std::string& url, uint16_t location ) {
      require_auth( producer );
      check( url.size() < 512, "url too long" );
   ...
   void system_contract::unregprod( const name& producer ) {
      ...
      _producers.modify( prod, same_payer, [&]( producer_info& info ){
         ...
         info.is_producer = false;
      });
      accounts_activity accountsact(get_self(), producer.value);
      auto prod_act = accountsact.find( producer.value );
      if(prod_act != accountsact.end()){
         accountsact.modify( prod_act, producer, [&](account_activity& prd_info){
            prd_info.is_active_prod = false;
         });
      }
   }
   ...
   void system_contract::update_elected_producers( const block_timestamp& block_time ) {
      ...
      std::vector<inery::producer_authority> producers;
   ...
   void system_contract::makeprod( const name& voter_name, const name& proxy, const std::vector<name>& producers ) {
      ...
      // check( memory_check>=5242880000000, "You don't have a memory to start making blocks." );
      
      auto prod = _producers.find( voter_name.value );
      //check( !(prod->is_producer), "producer is already active." );
      ...
      // _gstate.last_validator_added = ct;
      accounts_activity accountsact(get_self(), voter_name.value);
      auto prod_act = accountsact.find( voter_name.value );
      if (prod_act != accountsact.end()){
         accountsact.modify( prod_act, voter_name, [&](account_activity& prd_info){
            prd_info.is_active_prod = true;
         });
      }
   ...
   void system_contract::update_votes( const name& voter_name, const name& proxy, const std::vector<name>& producers, bool voting ) {
      ...
      for( const auto& pd : producer_deltas ) {
         ...
         check( voter->owner.to_string()==pitr->owner.to_string(), " You can only vote for yourself." );
