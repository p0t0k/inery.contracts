 ...
 * INERY Contracts
 *
 * The design of the Inery blockchain calls for a number of smart contracts that are run at a
 * privileged permission level in order to support functions such as block producer registration and
 * voting, token staking for CPU and network bandwidth, MEM purchasing, multi-sig, etc. These smart
 ...
 * managing, and/or using an Inery blockchain. They are provided for reference purposes:
 * - inery.bios
 * - inery.system
 * - inery.msig
 * - inery.wrap
...
namespace inerybios {
   ...
   class [[inery::contract("inery.bios")]] bios : public inery::contract {
      public:
          ...
          * therefore, this method will execute an inline buyresources from receiver for newacnt.
          */
         [[inery::action]]
          ...
          */
         [[inery::action]]
         void setalimits( name account, int64_t mem_bytes, int64_t net_weight, int64_t cpu_weight );
          ...
          */
         [[inery::action]]
         void setprods( const std::vector<inery::producer_authority>& schedule );
          ...
          */
         [[inery::action]]
         void reqactivated( const inery::checksum256& feature_digest );

         struct [[inery::table]] abi_hash {
            name              owner;
            checksum256       hash;
         ...
         typedef inery::multi_index< "abihash"_n, abi_hash > abi_hash_table;
