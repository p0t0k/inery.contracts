...
### Indirect method using inery.msig contract

Cline currently provides tools to propose an action with the inery.msig contract, but it does not provide an easy interface to propose a custom transaction.

So, at the moment it is difficult to propose an atomic transaction with multiple actions (for example `inery::setcode` followed by `inery::setabi`).

The advantage of the inery.msig method is that it makes coordination much easier and does not place strict time limits (less than 9 hours) on signature collection.

The disadvantage of the inery.msig method is that it requires the proposer to have sufficient RAM to propose the transaction and currently cline does not provide convenient tools to use it with custom transactions like the one that would be necessary to atomically upgrade the system contract.

For now, it is recommended to use the direct method to upgrade the system contract.

### Direct method (avoids using inery.msig contract)
...
```
$ cline get code -c original_system_contract.wast -a original_system_contract.abi inery
...
```
$ cline set contract -s -j -d inery contracts/inery.system | tail -n +4 > upgrade_system_contract_trx.json
```
...
First, the block producer should collect all the necessary information. Let us assume that the block producers active key pair is `(INE5kBmh5kfo6c6pwB8j77vrznoAaygzoYvBsgLyMMmQ9B6j83i9c, 5JjpkhxAmEfynDgSn7gmEKEVcBqJTtu6HiQFf4AVgGv5A89LfG3)`. The block producer needs their active private key (`5JjpkhxAmEfynDgSn7gmEKEVcBqJTtu6HiQFf4AVgGv5A89LfG3` in this example), the `upgrade_system_contract_official_trx.json`, and the `chain_id` (`d0242fb30b71b82df9966d10ff6d09e4f5eb6be7ba85fd78f796937f1959315e` in this example) which can be retrieved through `cline get info`.
...
```
$ cline sign --chain-id d0242fb30b71b82df9966d10ff6d09e4f5eb6be7ba85fd78f796937f1959315e upgrade_system_contract_trx.json | tail -n 5
...
```
$ cline push transaction --skip-sign upgrade_system_contract_official_trx_signed.json
{
...
```
$ cline get code -c new_system_contract.wast -a new_system_contract.abi inery
