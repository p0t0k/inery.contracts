...
## 1. Installing the inery.wrap contract

The inery.wrap contract needs to be installed on a privileged account to function. It is recommended to use the account `inery.wrap`.

First, the account `inery.wrap` needs to be created. Since it has the restricted `inery.` prefix, only a privileged account can create this account. So this guide will use the `inery` account to create the `inery.wrap` account. On typical live blockchain configurations, the `inery` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `inery.msig` contract to help coordinate the approvals of the proposed transaction that creates the `inery.wrap` account.

The `inery.wrap` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`inery`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `inery` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `inery` account.

This guide will be using cline to carry out the process.

### 1.1 Create the inery.wrap account

#### 1.1.1 Generate the transaction to create the inery.wrap account

The transaction to create the `inery.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cline command to propose the transaction to the inery.msig contract.

A simple way to generate a transaction to create a new account is to use the `cline system newaccount`. However, that sub-command currently only accepts a single public key as the owner and active authority of the new account. However, the owner and active authorities of the new account should only be satisfied by the `active` permission of `inery`. One option is to create the new account with the some newly generated key, and then later update the authorities of the new account using `cline set account permission`. This guide will take an alternative approach which atomically creates the new account in its proper configuration.

Three unsigned transactions will be generated using cline and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the inery.msig contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```
$ cline system newaccount -s -j -d --transfer --stake-net "1.000 SYS" --stake-cpu "1.000 SYS" --buy-ram-kbytes 50 inery inery.wrap INE8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"inery","action":"buyrambytes","args":{"payer":"inery","receiver":"inery.wrap","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"inery","action":"delegatebw","args":{"from":"inery","receiver":"inery.wrap","stake_net_quantity":"1.0000 SYS","stake_cpu_quantity":"1.0000 SYS","transfer":true}}
...
The actual public key used is not important since that data is only encoded into the `inery::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `inery::newaccount` action. It should look like:
```
$ cat newaccount_payload.json
{
   ...
   "owner": {
      ...
      "keys": [],
      "accounts": [{
         "permission": {"actor": "inery", "permission": "active"},
         "weight": 1
      }],
      "waits": []
   },
...
Third, generate a transaction containing the actual `inery::newaccount` action that will be used in the final transaction:
```
$ cline push action -s -j -d inery newaccount newaccount_payload.json -p inery > generated_newaccount_trx.json
...
Fourth, generate a transaction containing the `inery::setpriv` action which will make the `inery.wrap` account privileged:
```
$ cline push action -s -j -d inery setpriv '{"account": "inery.wrap", "is_priv": 1}' -p inery > generated_setpriv_trx.json
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the inery.msig contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_wrap_account_trx.json) and edit the first three fields so it looks something like the following:
```
...
It will be useful to have a JSON of the active permissions of each of the active block producers for later when proposing transactions using the inery.msig contract.
...
#### 1.1.2 Propose the transaction to create the inery.wrap account
...
```
$ cline multisig propose_trx createwrap producer_permissions.json create_wrap_account_trx.json blkproducera
executed transaction: bf6aaa06b40e2a35491525cb11431efd2b5ac94e4a7a9c693c5bf0cfed942393  744 bytes  772 us
#    inery.msig <= inery.msig::propose          {"proposer":"blkproducera","proposal_name":"createwrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 1.1.3 Review and approve the transaction to create the inery.wrap account
...
The proposed transaction can be reviewed using the `cline multisig review` command:
```
$ cline multisig review blkproducera createwrap > create_wrap_account_trx_to_review.json
...
```
$ cline multisig propose_trx -j -s -d createwrap '[]' create_wrap_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_wrap_trx_serialized.hex
...
```
$ cline multisig approve blkproducera createwrap '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 03a907e2a3192aac0cd040c73db8273c9da7696dc7960de22b1a479ae5ee9f23  128 bytes  472 us
#    inery.msig <= inery.msig::approve          {"proposer":"blkproducera","proposal_name":"createwrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 1.1.4 Execute the transaction to create the inery.wrap account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `inery.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the inery.msig contract).

```
$ cline multisig exec blkproducera createwrap blkproducera
executed transaction: 7ecc183b99915cc411f96dde7c35c3fe0df6e732507f272af3a039b706482e5a  160 bytes  850 us
#    inery.msig <= inery.msig::exec             {"proposer":"blkproducera","proposal_name":"createwrap","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `inery.wrap` was created:
```
$ cline get account inery.wrap
privileged: true
permissions:
     owner     1:    1 inery@active,
        active     1:    1 inery@active,
memory:
...
### 1.2 Deploy the inery.wrap contract

#### 1.2.1  Generate the transaction to deploy the inery.wrap contract

The transaction to deploy the contract to the `inery.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cline command to propose the transaction to the inery.msig contract.

The easy way to generate this transaction is using cline:
```
$ cline set contract -s -j -d inery.wrap contracts/inery.wrap/ > deploy_wrap_contract_trx.json
Reading WAST/WASM from contracts/inery.wrap/inery.wrap.wasm...
...
#### 1.2.2 Propose the transaction to deploy the inery.wrap contract
...
```
$ cline multisig propose_trx deploywrap producer_permissions.json deploy_wrap_contract_trx.json blkproducera
executed transaction: 9e50dd40eba25583a657ee8114986a921d413b917002c8fb2d02e2d670f720a8  4312 bytes  871 us
#    inery.msig <= inery.msig::propose          {"proposer":"blkproducera","proposal_name":"deploywrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 1.2.3 Review and approve the transaction to deploy the inery.wrap contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `cline multisig review` command:
```
$ cline multisig review blkproducera deploywrap > deploy_wrap_contract_trx_to_review.json
...
```
$ cline multisig propose_trx -j -s -d deploywrap '[]' deploy_wrap_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_wrap_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ cline multisig approve blkproducera deploywrap '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: d1e424e05ee4d96eb079fcd5190dd0bf35eca8c27dd7231b59df8e464881abfd  128 bytes  483 us
#    inery.msig <= inery.msig::approve          {"proposer":"blkproducera","proposal_name":"deploywrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 1.2.4 Execute the transaction to create the inery.wrap account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `inery.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the inery.msig contract).

```
$ cline multisig exec blkproducera deploywrap blkproducera
executed transaction: e8da14c6f1fdc3255b5413adccfd0d89b18f832a4cc18c4324ea2beec6abd483  160 bytes  1877 us
#    inery.msig <= inery.msig::exec             {"proposer":"blkproducera","proposal_name":"deploywrap","executer":"blkproducera"}
```

Anyone can now verify that the `inery.wrap` contract was deployed correctly.

```
$ cline get code -a retrieved-inery.wrap.abi inery.wrap
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-inery.wrap.abi
$ sha256sum contracts/inery.wrap/inery.wrap.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/inery.wrap/inery.wrap.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-inery.wrap.abi, can then be compared to the original ABI of the contract (contracts/inery.wrap/inery.wrap.abi) to ensure they are semantically the same.

## 2. Using the inery.wrap contract

### 2.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed inery.wrap contract together with the inery.msig contract to allow a greater than two-thirds supermajority of block producers of an INERY blockchain to change the owner authority of an arbitrary account. The example will use cline: in particular, the `cline multisig` command, the `cline set account permission` sub-command, and the `cline wrap exec` sub-command. However, the guide also demonstrates what to do if the `cline wrap exec` sub-command is not available.
...
Assume that none of the block producers know the private key corresponding to the public key `INE6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV` which, as can be seen above, is initially securing access to the `alice` account.
...
```
$ cline set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "inery", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
The next step is to generate the transaction containing the `inery.wrap::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ cline wrap exec -s -j -d blkproducera update_alice_owner_trx.json > wrap_update_alice_owner_trx.json
```
...
```
$ cline multisig propose_trx -s -j -d nothing '[]' update_alice_owner_trx.json nothing | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > update_alice_owner_trx_serialized.hex
...
Then generate the template for the transaction containing the `inery.wrap::exec` action:
```
$ cline push action -s -j -d inery.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
$ cat wrap_update_alice_owner_trx.json
{
...
```
$ cline multisig propose_trx updatealice producer_permissions.json wrap_update_alice_owner_trx.json blkproducera
executed transaction: 10474f52c9e3fc8e729469a577cd2fc9e4330e25b3fd402fc738ddde26605c13  624 bytes  782 us
#    inery.msig <= inery.msig::propose          {"proposer":"blkproducera","proposal_name":"updatealice","requested":[{"actor":"blkproducera","permi...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ cline multisig review blkproducera updatealice > wrap_update_alice_owner_trx_to_review.json
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodine/cline, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `inery.wrap::exec` action is available in the output of the `cline multisig review` command, only the hex data is available of the payload of the inner `inery::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in nodine/cline, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `inery::setcode` action to set the contract code of the `inery.wrap` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
...
```
$ cline multisig propose_trx -j -s -d updatealice '[]' wrap_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_wrap_update_alice_owner_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ cline multisig approve blkproducera updatealice '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 2bddc7747e0660ba26babf95035225895b134bfb2ede32ba0a2bb6091c7dab56  128 bytes  543 us
#    inery.msig <= inery.msig::approve          {"proposer":"blkproducera","proposal_name":"updatealice","level":{"actor":"blkproducerb","permission...
warning: transaction executed locally, but may not be confirmed by the network yet
```
